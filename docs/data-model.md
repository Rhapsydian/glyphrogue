...
| `setLayerFont(ctx, metrics, fontFamily)` / `drawGlyphCell(ctx, metrics, cellSize, { col, row, text, color })` | the only functions touching `ctx.font`/`ctx.fillStyle`/`ctx.fillText`; `color` is an opaque value/token passed straight through, palette-token resolution is session 22's job | `packages/core/src/glyphRenderer.js` | implemented |
...
| `classifyVisibility` / `updateRemembered` / `computeLighting` / `cellRenderState` / `updateLastKnownLight` / `MEMORY_TONE` | pure FOV/lighting visualization over `computeFov` — visible/remembered/unknown classification, light-source blending (summed intensity, highest-intensity source's color), and the `preserveLastKnownLight` config toggle `rendering.md` calls for. `MEMORY_TONE` is a sentinel resolved to an actual color by session 22 | `packages/core/src/visibility.js` | implemented |
